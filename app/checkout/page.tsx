...
import { FiCheckCircle, FiTruck, FiMapPin, FiCreditCard, FiLock, } from 'react-icons/fi';
...
export default function CheckoutPage() {
  ...
  const [payment, setPayment] = useState<{ order: any; clientSecret: string } | null>(null);
  ...
  if (orderSuccess) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-12 sm:py-20 text-center space-y-6">
        <div className="bg-card border border-emerald-200 p-6 sm:p-10 rounded-3xl space-y-4 shadow-xl">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground break-words">
            ...
            <strong className="text-brand-red font-mono font-bold text-sm">{orderSuccess.orderNumber}</strong>.
          </p>

          <div className="bg-muted p-4 rounded-2xl border border-border text-xs space-y-2 text-left text-muted-foreground max-w-md mx-auto">
            <div className="flex justify-between gap-3">
              <span>Total Paid:</span>
              <span className="font-bold font-mono text-emerald-400">£{orderSuccess.totalAmount.toFixed(2)}</span>
            </div>
            <div className="flex justify-between gap-3">
              <span className="shrink-0">Method:</span>
              <span className="font-bold text-white text-right">
                {orderSuccess.shippingMethod === 'PICKUP_BRADFORD' ? 'Bradford HQ Pickup' : `Freight Delivery (${orderSuccess.shippingCountry})`}
              </span>
            </div>
            <div className="flex justify-between gap-3">
              <span>Status:</span>
              <span className="font-bold text-brand-red">{orderSuccess.status}</span>
            </div>
          </div>
          ...
          <div className="pt-4 flex justify-center">
            <button
              ...
              className="w-full sm:w-auto bg-primary hover:opacity-90 text-primary-foreground font-bold text-xs px-6 py-3.5 rounded-xl shadow min-h-[48px]"
            >
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-6 sm:space-y-8">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">Complete your shipping & payment details</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* Right Column: Order Review & Stripe Card Payment */}
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-card border border-border p-4 sm:p-6 rounded-2xl space-y-4 shadow-xl">
            <h3 className="font-bold text-card-foreground text-base border-b border-border pb-3">Payment & Final Review</h3>
            ...
            {!payment && (
              <>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="btn-primary min-h-[52px] text-sm"
                >
            ...
            {payment && (
              <div className="bg-muted border border-border p-4 rounded-xl space-y-3 text-xs">
                <div className="flex items-center justify-between text-foreground font-bold border-b border-border pb-2">
